...
export function BettingHistory({ gameId, players }: BettingHistoryProps) {
  ...
  useEffect(() => {
    async function fetchActions() {
      const { data, error } = await supabase
        ...
        .select('*')
        .eq('game_id', gameId)
        .in('action_type', ['bet', 'call', 'raise', 'half', 'quarter', 'check', 'die'])
    ...
    const subscription = supabase
      ...
      .on('postgres_changes', {
        ...
        if (payload.new && ['bet', 'call', 'raise', 'half', 'quarter', 'check', 'die'].includes(payload.new.action_type)) {
  ...
  function getActionText(action: ActionItem): string {
    switch (action.action_type) {
      ...
      case 'quarter':
        return `${action.amount?.toLocaleString() || 0}원 따당`
      case 'check':
        return '체크'
      case 'die':
        return '다이'
