...
export async function processBetting(
  ...
  try {
    console.log(`[processBetting] Player ${playerId} in game ${gameId} is taking action: ${action} with amount: ${amount}`);
    ...
    if (!gameData) {
      throw handleGameError(null, ErrorType.NOT_FOUND, '게임을 찾을 수 없습니다');
    }
    ...
    if (!playerData) {
      throw handleGameError(null, ErrorType.NOT_FOUND, '플레이어를 찾을 수 없습니다');
    }
    
    // 현재 턴이 요청한 플레이어의 턴인지 확인
    if (gameData.current_player_id !== playerId) {
      throw handleGameError(null, ErrorType.INVALID_STATE, '현재 당신의 턴이 아닙니다');
    }
    ...
    switch (action) {
      case BettingAction.CALL:
        ...
        if (gameData.current_bet_amount > 0) {
          const callAmount = Math.min(gameData.current_bet_amount - playerData.current_bet, playerData.chips);
      ...
      case BettingAction.RAISE:
        ...
        if (amount <= gameData.current_bet_amount) {
          throw handleGameError(null, ErrorType.VALIDATION, '레이즈 금액은 현재 배팅보다 커야 합니다');
        }
        
        if (amount > playerData.chips) {
          throw handleGameError(null, ErrorType.VALIDATION, '보유 칩보다 더 많이 배팅할 수 없습니다');
        }
      ...
      case BettingAction.CHECK:
        ...
        if (gameData.current_bet_amount > 0 && playerData.current_bet < gameData.current_bet_amount) {
          throw handleGameError(null, ErrorType.INVALID_STATE, '현재 베팅이 있을 때는 체크할 수 없습니다');
        }
        nextAction = 'checked';
        break;
      ...
      default:
        throw handleGameError(null, ErrorType.VALIDATION, '유효하지 않은 배팅 액션입니다');
    }
    ...
    const { error: transactionError } = await supabase
      ...
      .insert([{
        ...
        player_id: playerId,
        action: action,
        amount: action === BettingAction.ALLIN ? playerData.chips : amount,
        round: gameData.round || 1
      }]);
    
    if (transactionError) {
      throw handleDatabaseError(transactionError, '배팅 트랜잭션 기록 실패');
    }
    ...
    const { error: updatePlayerError } = await supabase
      ...
      .update({
        ...
        current_bet: action === BettingAction.RAISE ? amount : 
                      action === BettingAction.CALL ? gameData.current_bet_amount : 
    ...
    if (updatePlayerError) {
      throw handleDatabaseError(updatePlayerError, '플레이어 정보 업데이트 실패');
    }
    ...
    const { data: updatedGame, error: updateGameError } = await supabase
      ...
      .update({
        ...
        current_bet_amount: action === BettingAction.RAISE ? amount : gameData.current_bet_amount,
        last_action: `${playerData.username} ${nextAction} ${amount > 0 ? amount + ' chips' : ''}`
      })
      .eq('id', gameId)
      .select('*, room:rooms(*)')
      .single();
...
export async function getNextPlayerTurn(gameId: string, currentPlayerId: string): Promise<string> {
  try {
    ...
    if (!players || players.length === 0) {
      throw handleGameError(null, ErrorType.INVALID_STATE, '활성 플레이어가 없습니다');
    }
...
export async function checkRoundCompletion(gameId: string): Promise<void> {
  try {
    ...
    const allPlayersMatchBet = playersWithChips.every(p => 
      p.current_bet === currentBet || p.chips === 0 // 올인한 플레이어는 예외
    );
...
export async function endRound(gameId: string): Promise<void> {
  try {
    ...
    // 승자에게 팟 분배
    if (winnerId) {
      console.log(`[endRound] Winner is ${winnerUsername} (${winnerId}) - awarding pot: ${game.pot}`);
    ...
    const { error: updateGameError } = await supabase
      ...
      .update({
        ...
        last_action: winnerId ? `${winnerUsername} won the pot (${game.pot} chips)` : 'Round ended',
...
export async function dealNewRound(gameId: string): Promise<void> {
  try {
    ...
    for (let i = 0; i < players.length; i++) {
      ...
      if (updatePlayerError) {
        console.error(`[dealNewRound] Error updating player ${players[i].id}:`, updatePlayerError);
      }
    }
    ...
    if (players.length > 0) {
      ...
      if (updateGameError) {
        console.error(`[dealNewRound] Error updating game state:`, updateGameError);
      }
    }
...
export async function handleBettingTimeout(
  ...
  console.log(`[handleBettingTimeout] Processing timeout for player ${playerId} in game ${gameId}`);
  
  try {
    // 타임아웃 발생시 자동 폴드 처리
    await processBetting(gameId, playerId, BettingAction.FOLD, 0);
    console.log(`[handleBettingTimeout] Auto-folded player ${playerId} due to timeout`);
  } catch (error) {
