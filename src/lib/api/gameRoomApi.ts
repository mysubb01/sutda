...
export async function createGame(
  ...
  try {
    ...
    throw handleGameError(error, ErrorType.DATABASE, "게임 생성 중 오류");
  }
}
...
export async function joinGame(
  ...
  try {
    ...
    if (gameData.status !== "waiting") {
      console.error(
        `[joinGame] Game not in waiting state: ${gameId}, status: ${gameData.status}`
      );
    ...
    if (seatIndex !== undefined) {
      ...
      // 요청 좌석 점유 확인
      // 아직 playerId가 생성되지 않았으므로 현재 플레이어 ID 없이 점유 확인
...
export async function toggleReady(
  ...
  try {
    ...
    console.error(
      `[toggleReady] Error for player ${playerId} in game ${gameId}:`,
      error
    );
    if (error.name === "GameError") {
      return {
...
export async function canStartGame(
  ...
  try {
    ...
    return {
      canStart: false,
      message: "게임 시작 가능 여부 확인 중 오류가 발생했습니다.",
    };
  }
}
