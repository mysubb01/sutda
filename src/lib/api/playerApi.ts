  ...
  try {
    ...
    if (storedPlayerId) {
      ...
      if (!existingPlayerError && existingPlayer) {
        console.log(`[joinGame] Rejoining as existing player: ${existingPlayer.id}, seat: ${existingPlayer.seat_index}`);
        ...
        console.log(`[joinGame] Stored player not found in database: ${storedPlayerId}`);
    ...
    if (gameData.status !== 'waiting') {
      console.error(`[joinGame] Game is not in waiting state: ${gameData.status}`);
      throw handleGameError(null, ErrorType.INVALID_STATE, '게임이 이미 시작되었거나 종료되었습니다.');
    }
    ...
    if (seatIndex !== undefined) {
      ...
      if (isOccupied) {
        console.error(`[joinGame] Requested seat ${seatIndex} is already occupied`);
        throw handleGameError(null, ErrorType.INVALID_STATE, `좌석 ${seatIndex}번은 이미 다른 플레이어가 사용 중입니다.`);
      }
    ...
    // 2. 자동 좌석 할당이 요청된 경우
      ...
      if (finalSeatIndex === null) {
        ...
        throw handleGameError(null, ErrorType.GAME_FULL, '게임에 더 이상 빈 좌석이 없습니다.');
      }
    }
    
    console.log(`[joinGame] Creating player - ID: ${playerId}, seat: ${finalSeatIndex}`);
    ...
    try {
      ...
      console.log(`[joinGame] Player created successfully: ${playerId}, seat: ${finalSeatIndex}`);
...
export async function getGamePlayers(
  ...
  try {
    ...
    console.log(`[getGamePlayers] Found ${data.length} players for game: ${gameId}`);
