...
export async function getAllRooms(): Promise<Room[]> {
  ...
  return roomsWithPlayerCount.filter((room) => {
    ...
    if (
      roomName.includes("debug") ||
      roomName.includes("디버그") ||
      roomName.includes("test") ||
      roomName.includes("테스트")
    ) {
      return false;
    }
...
export async function createRoom(
  ...
  try {
    ...
    const { error: gameError } = await supabase.from("games").insert({
      ...
      game_mode: mode, // 누락 필드 추가
      betting_end_time: null, // 누락 필드 추가
      card_selection_time: null, // 누락 필드 추가
    });
    ...
    if (playerError) {
      ...
      throw new Error(
        "방장 플레이어를 생성할 수 없습니다: " + playerError.message
      );
    }
  ...
  try {
    ...
    const { data: gameData, error: gameError } = await supabase
      ...
      .insert({
        ...
        game_mode: roomData.mode, // 누락 필드 추가
        betting_end_time: null, // 누락 필드 추가
        card_selection_time: null, // 누락 필드 추가
      });
...
export async function changeSeat(
  ...
  console.log(
    `[DEPRECATED] changeSeat in roomApi.ts is called. Use updateSeat from gameApi.ts instead`
  );
  ...
  try {
    ...
    if (inactivePlayers && inactivePlayers.length > 0) {
      for (const player of inactivePlayers) {
        console.log(
          `플레이어 ${player.username} (${player.id})을(를) 비활성화 처리`
        );
